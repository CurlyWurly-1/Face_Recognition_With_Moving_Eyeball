...
#define X1SERVO 8 // X Servo (left-right) on right eye
#define Y1SERVO 9 // Y Servo (up-down) on right eye
#define L1SERVO 10 // Bottom Eyelid, controlled by right-side eyelid servo

#define X2SERVO 12 // X Servo (left-right) on left eye
#define Y2SERVO 13 // Y Servo (up-down) on left eye
#define L2SERVO 14 // Top Eyelid, controlled by left-side eyelid servo
...
float X1DegComp = 18;    // it's Left  Eye across (more POS and it's Left  eye looks to it's left)
float X2DegComp = 11;    // it's Right Eye across (more POS and it's Right eye looks to it's left)
float Y1DegComp = 23;    // it's Left  Eye up     (more POS and it's Left  eye goes up)
float Y2DegComp = 8;     // it's Right Eye up     (more POS and it's Right eye goes up)
float L1DegComp = -25;
float L2DegComp = -44;
...
unsigned long timeLastUpdate = millis();
int timeInterval = 5000;
int eyeStatus = 0;   // 0=set it closed, 1=closed,  2=Open 
...
//    pinMode(LaserPin, OUTPUT);
    delay(10);
    timeLastUpdate = millis() - timeInterval;
}
...
void loop() {
    ...
    if ( ( eyeStatus != 1 ) and ( millis() - timeLastUpdate ) > timeInterval ) {
      eyeStatus = 1;
      XFromPi = 0; 
      YFromPi = 0;    
      L1FromPi = 90;    
